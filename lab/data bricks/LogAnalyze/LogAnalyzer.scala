...
object LogAnalyzer extends App {
  ...
  val sparkConf = new SparkConf().setAppName("Log Analyzer in Scala").setMaster("local[*]")
  ...
  println(s"""Top Endpoints(1): ${topEndpoints.mkString("[", ",", "]")}""")

   // Top Endpoints2.
  val topEndpoints2 = accessLogs
    .map(_.endpoint -> 1L)
    .reduceByKey(_ + _)
    .sortBy(_._2, false).take(10)
  println(s"""Top Endpoints(2): ${topEndpoints2.mkString("[", ",", "]")}""")
  sc.stop()
}